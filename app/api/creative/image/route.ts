import { ImageGenerationService, ImageGenerationOptions } from '@/lib/creative';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { provider, modelId, prompt, size, quality, style, negativePrompt, preserveFacialFeatures } = body;
    ...
    // Generate the image
    const result = await ImageGenerationService.generateImage(options, userId);
